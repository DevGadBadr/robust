(function () {
  ...
  function buildCandidates(el) {
    var out = [];
    if (!el || el.nodeType !== 1 || !el.isConnected) return out;
    ...
    function add(type, value, score, fallback) {
      if (value) out.push({ type: type, value: value, score: score, fallback: !!fallback });
    }
    ...
    for (var i = 0; i < out.length; i++) {
      ...
      seen[key] = true;
      var info = matchInfo(root, candidate.type, candidate.value, el);
      if (!info) continue;
      candidate.count = info.count;
      if (info.count > 1) candidate.score -= 12;
  ...
  function contextFor(hosts) {
    ...
    if (hosts && hosts.length) {
      ...
      for (var i = 0; i < hosts.length; i++) {
        ...
        var selector = shortSelectorFor(host, hostRoot) || fullCssPath(host, hostRoot);
  ...
  function showChooser(el, hosts, x, y) {
    ...
    if (!candidates.length) {
      ...
      candidates.forEach(function (candidate) {
        ...
        meta.textContent = candidate.count > 1
          ? "(" + candidate.score + ", 1 of " + candidate.count + ")"
          : "(" + candidate.score + ")";
    ...
    cancelRow.addEventListener("click", function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
  ...
  function blockEvent(ev) {
    ...
    if (isMenuNode(ev.target)) return;
